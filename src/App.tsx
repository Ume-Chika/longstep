...
const helpTopics: HelpTopic[] = [
  ...
  },
  {
    id: 'faq',
    label: '6. よくある質問（FAQ）',
    summary: '道筋の引き方、カード記号、複数プロジェクト、AI連携、同期など。',
    heading: 'よくある質問（FAQ）',
    steps: [
      {
        title: '道筋はどう繋ぐのが正しい？',
        body: <>どちらかでも進められる作業は<strong>「分岐」</strong>させ、両方が終わってからでないとできない目標へは<strong>「合流」</strong>させます。カード右端の「✎」をドラッグすることで自由に道筋を繋げられます。</>,
      },
      {
        title: '黄色い枠やカードの記号の意味は？',
        body: <>黄色い枠は<strong>「前提が完了し今すぐ着手できる目標」</strong>もしくは<strong>「分割するべき目標」</strong>です。カード左の「＋」で前提追加、上部の「⋮⋮」で上下入れ替え、「◆ / ✦ / ·」は大中小の目標レベルを表します。</>,
      },
      {
        title: '複数のプロジェクトを管理できる？',
        body: <>できます。別のプロジェクトは画面右下の追加ボタンの<strong>「新規計画書を作成する」</strong>から作成してください。表示する計画書の切り替えはホームから可能です。</>,
      },
      {
        title: 'AIは目標の追加や道筋の接続もできる？',
        body: <>できます。進捗の更新だけでなく、「この目標を達成するための小目標を作って繋げて」と指示すれば、目標の細分化や道筋の接続までAIが自動で構築します。</>,
      },
      {
        title: '別の作業フォルダでも同じ計画を操作できる？',
        body: <>できます。<strong>ホーム画面で計画を右クリック→「AI連携ファイルを追加」→作業したい各プロジェクトフォルダを選択</strong>で、AI連携ファイルを追加できます。複数のフォルダからAIエージェントがあなたの計画書を編集できるようになります。</>,
      },
      {
